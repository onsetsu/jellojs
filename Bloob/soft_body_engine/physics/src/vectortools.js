var VectorTools = {
	...
	/*,
	"calculateSpringForceDirLength": function(dir, length, velA, velB, springD, springK, damping) {
		var dist = springD - length;
		
		var relVel = new Vector2(velA.sub(velB));
		var totalRelVel = relVel.dotProduct(dir);
		
		//printf("calculateSpringForce: dist[%f] k[%f]  totalRelVel[%f] damp[%f]\n", dist, springK, totalRelVel, damping);
		
		return dir.mulFloat((dist * springK) - (totalRelVel * damping)); 		
	}*/
};
