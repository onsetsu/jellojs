...
PressureBody.prototype.accumulateInternalForces = function() {
	...
	for (var i = 0; i < this.mPointCount; i++)
	{
		...
		var pmP = this.mPointMasses[prev]; // PointMass&
		var pmI = this.mPointMasses[i]; // PointMass&
		var pmN = this.mPointMasses[next]; // PointMass&
	...
	for (var i = 0; i < this.mPointCount; i++)
	{
		...
		this.mPointMasses[i].Force.addSelf(this.mNormalList[i].mulFloat(pressureV));
		this.mPointMasses[j].Force.addSelf(this.mNormalList[j].mulFloat(pressureV));
	}
}
